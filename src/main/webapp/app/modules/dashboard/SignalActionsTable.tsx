...
const SignalActionsTable = () => {
  ...
  const playNotificationSound = async () => {
    if (!audioEnabled) {
      console.log('🔇 Audio not enabled - click Enable Sound first');
      return;
    }

    try {
      console.log(`🔊 Playing ${audioSettings.pattern} sound for ${audioSettings.duration}s...`);
      ...
      switch (audioSettings.pattern) {
        case 'ding':
          await playDingPattern(audioContext);
          break;
        case 'chime':
          await playChimePattern(audioContext);
          break;
        case 'bell':
          await playBellPattern(audioContext);
          break;
        case 'melody':
          await playMelodyPattern(audioContext);
          break;
        default:
          await playDingPattern(audioContext);
      }

      console.log('✅ Custom notification sound completed!');
                  ...
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map(signal => (
                      <tr key={signal.id} className={isRowHighlighted(signal) ? 'row-highlighted' : ''}>
                        ...
                        <td className="annotation">{isRowHighlighted(signal) && <span className="new-signal-badge">🆕</span>}</td>
                      </tr>
                    ))
                  )}
                </tbody>
